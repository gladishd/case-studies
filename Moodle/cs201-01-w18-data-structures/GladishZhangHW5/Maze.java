...
public class Maze {
    ...
    public MazeSquare getNeighbor(MazeSquare s, String direction) { 
        ...
        // have a mazeSquare as an input and get the  mazesquare at the direction of the input mazeSquare
         ...
         */
        int tempRow; // count for the row of the mazeSquare
        int tempColumn; // count for the column of the mazeSquare
        int rowIndex = 0; // count for the row of the the neighbor mazeSquare of the input mazeSquare
        int colIndex = 0; // count for the column of the the neighbor mazeSquare of the input mazeSquare

        /**
         * First, we need to find where s is located
         * within the doubly nested array rowList.
         */
        for (int row = 0; row < rowList.size(); row++) { // for each row starting at row 0
            for (int column = 0; column < rowList.get(0).size(); column++) { // for each column starting at column 0
                /**
                 ...
                 * of this location.
                 */
                if (rowList.get(row).get(column) == s) { // if the input is the start mazeSquare
                    tempRow = row; // record the row of it
                    tempColumn = column; // record the column of it
                    /**
                     ...
                     */
                    if (direction == "left") { // if neighbor is on the left
                        rowIndex = tempRow;  // it is on the same row
                        colIndex = tempColumn - 1;  // it is one mazeSquare left
        ...
        // once we have the location of the neigbhor, try to get mazeSquare in the maze
    }
    ...
    public LLStack<MazeSquare> getSolution() {
        ...
        int startRow = 100; 
        int startCol = 300;
        // intializing the varibales to location the starting square so that we can push it to the stack
        
        for (int row = 0; row < rowList.size(); row++) {
            for (int column = 0; column < rowList.get(0).size(); column++) {
               if (rowList.get(row).get(column).isStart()) {
                  startRow = row;
                  startCol = column; // locate and record the starting square
               }
            }
        }
        emptyStack.push(rowList.get(startRow).get(startCol)); // push it to the stack 
        rowList.get(startRow).get(startCol).visit(); // mark it as visited
        ...
        while (!emptyStack.isEmpty() && !emptyStack.peek().isEnd()) { // if the top of the stack is not the final square or the maze is solvable
            T = (MazeSquare) emptyStack.peek(); // make t the top of the stack
            ...
            if (T.r == rowList.size() - 1 && T.c == currRow.size() - 1) { // if the top square is on the lower right corner
                MazeSquare n1 = getNeighbor(T, "left");  // make n1 the left square of the top square
                MazeSquare n2 = getNeighbor(T, "top");   // make n1 the top square of the top square 
                                                         // since it is on the lower right corner, there is no bottom one or right one 
                if ((n1.hasRightWall() || n1.isVisited()) &&     // if n1 and n2 can not be visited or have been visited
                        (n2.hasBottomWall() || n2.isVisited())) {
                    emptyStack.pop();                            // pop the T
                } else if (!(n1.hasRightWall() || n1.isVisited())) {  // if n1 is available
                    n1.visit();
                    emptyStack.push(n1);                         //  mark n1 as visited and push it to the stack
                } else if (!(n2.hasBottomWall() || n2.isVisited())) { // if n2 is available
                    n2.visit();
                    emptyStack.push(n2);                         //  mark n2 as visited and push it to the stack
                }
            } else if (T.r == rowList.size() - 1 && T.c == 0) { // if the top square is on the lower left corner
                MazeSquare n1 = getNeighbor(T, "right"); // do the same thing except taht n1 and n2 are the right and top of T
            ...
            } else if (T.r == 0 && T.c == currRow.size() - 1) { // if the top square is on the upper right corner
            ...
            } else if (T.r == 0 && T.c == 0) { // if the top square is on the upper left corner
             ...
             */
                ...
                if (T.r == 0) { //if the top square is on the top line of maze but not on the two corners along the top line
                    MazeSquare n1 = getNeighbor(T, "left");   // n1 and n2 and n3 are the left,right, and bottom squares
                    MazeSquare n2 = getNeighbor(T, "right");
                    MazeSquare n3 = getNeighbor(T, "bottom");
                    if ((n1.hasRightWall() || n1.isVisited()) &&   // if n1 and n2 and n3 have all been visited or can not be visited
                            (n2.hasLeftWall() || n2.isVisited()) &&
                            (n3.hasTopWall() || n3.isVisited())) {
                        emptyStack.pop();                         // pop T out of the stack
                    } else if (!n1.hasRightWall() && !n1.isVisited()) {  // if n1 is available
                        n1.visit();
                        emptyStack.push(n1);   // mark it as visited and push it to the stack
                    } else if (!(n2.hasLeftWall() || n2.isVisited())) {  // if n2 is available

                        n2.visit();
                        emptyStack.push(n2);
                    } else if (!(n3.hasTopWall() || n3.isVisited())) {   // if n3 is available

                        n3.visit();
                        emptyStack.push(n3);
                    }
                } else if (T.r == rowList.size() - 1) { // if T is alomg the bottom line of maze
                    MazeSquare n1 = getNeighbor(T, "left"); 
                    // do the same thing except that n1, n2, and n3 are left, right , and top squares of T
                ...
                } else if (T.c == 0) { // if T is alomg the left line of maze
                ...
                } else if (T.c == currRow.size() - 1) { // if T is alomg the right line of maze
                    MazeSquare n1 = getNeighbor(T, "left");
                    MazeSquare n2 = getNeighbor(T, "bottom");
                    MazeSquare n3 = getNeighbor(T, "top");
                    if ((n1.hasRightWall() || n1.isVisited()) &&
                     ...
                     */
                    MazeSquare n1 = getNeighbor(T, "left");  // all four adjacent squares are all potentially available
                    MazeSquare n2 = getNeighbor(T, "bottom");
                    MazeSquare n3 = getNeighbor(T, "top");
                    ...
                    // make a boolean that determine whether there is a viable square or not 
                    
                    if (noViableNeighbors) {  // if we have so unlucky that none of the four squares is viable
                        emptyStack.pop();     // pop the pathetic T out 
                    } else if (!(n1.hasRightWall() || n1.isVisited())) { 
                        // if n1 is viable, as it is the "first" one to examine
                        n1.visit();                                  // mark it as visited and push it to the stack
                        emptyStack.push(n1);
                    } else if (!(n2.hasTopWall() || n2.isVisited())) {
                        n2.visit();
                        emptyStack.push(n2);
        ...
        return emptyStack; // after doing all steps above, return the stack
    }
