...
/**
 * Class for QuickSort Investigation, exploring how changing the switch to insertion sort affects runtime.
 ...
 * You will likely need to modify the way in which the insertionSort threshold (currently set with MIN_SIZE) is specified.
 * 
...
public class QuickSort {
    ...
    private static void quickSort(int[] arr, int firstI, int lastI, int MIN_SIZE) {
        if (lastI - firstI <= MIN_SIZE) {
            ...
            int pivotI = partition(arr, firstI, lastI); // Partitions the array using the partition method.
            quickSort(arr, firstI, pivotI - 1, MIN_SIZE); // Since MIN_SIZE was static and final originally,
    ...
    private static int partition(int[] arr, int firstI, int lastI) {
        ...
        int pivotI = (lastI + firstI) / 2; // This generates the pivot's index in the array.
        int pivotV = arr[pivotI];          // This finds the pivot's value and sets it to pivotV.
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 1000; i += 10) {
            int insertionUpperThreshold = i; // This sets the size below which the quicksort method will resort to the use of insertion sort.
            int[] quicksortArray = new int[1000];
            long sortTime = 0;
            for (int j = 0; j < 30; j++) {
                // Due to the fact that we are looking to calculate precise values for small
                // changes in the insertionSort switch point, we must take an average of many values (30 in this
                // case).
                ShellSort.fillAndShuffle(quicksortArray); // Fills the array with values from 0 to size - 1 and shuffles it.
                long startTime = System.nanoTime(); // This ought to take the time in nanoseconds.

                quickSort(quicksortArray, insertionUpperThreshold); // The original test value was 10, so I use similar values.
                long endTime = System.nanoTime();
                sortTime += endTime-startTime;
            }
            sortTime = sortTime / 30; // Computes the average sort time for the given switch setting and array size.
            thresholdsArrayForSmallerArray.add("" + i);
            sortTimesArrayForSmallerArray.add("" + (int) sortTime); // We must cast sortTime (type long) to an integer.
        ...
        ArrayList<String> sortTimesArray = new ArrayList<String>();  // These two arrays will be used to make a scatter plot.

        //  The example of timing has been modified to create the following set of nested for-loops.
        /**
         * These loops essentially take the arrays listed above and add threshold and sorting times in milliseconds
         * to each of them respectively.
         */
        for (int j = 0; j < 1000000; j += 100000) {
            int insertionUpperThreshold = j; // This is the purpose of this for loop.  It runs the quickSort for
            // different switch (to insertionSort) thresholds.
            int[] quicksortArray = new int[1000000];
            long sortTime = 0;
            for (int i = 0; i < 3; i++) { // Provides a better measure (an average) of the typical sort time for a given
                                          // switch threshold.
                ShellSort.fillAndShuffle(quicksortArray); // Fills the array with values from 0 to size - 1 and shuffles it.
            ...
            sortTime = sortTime / 3; // Computes the average sort time for the given switch setting and array size.
            thresholdsArray.add("" + j);
            sortTimesArray.add("" + (int) sortTime); // We must cast sortTime (type long) to an integer.
            System.out.println(thresholdsArray);
            System.out.println(sortTimesArray);
            /**
             * I placed the prints to the console here so that we can monitor the progress of the arrays' creation.
             */
        }
    }   
}
